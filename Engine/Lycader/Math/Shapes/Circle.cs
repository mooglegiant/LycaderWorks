//-----------------------------------------------------------------------
// <copyright file="Circle.cs" company="Mooglegiant" >
...
namespace Lycader.Math.Shapes
{
    ...
    public struct Circle
    {
        ...
        {
            return Calculate.Distance(c1.center, c2.center) < c1.radius + c2.radius;
        }

        public bool Intersects(Circle circle)
        {
            return Calculate.Distance(this.center, circle.center) < this.radius + circle.radius;
        }

        public static bool IsInside(Circle circle, Vector2 point)
        {
            return Calculate.Distance(circle.center, point) < circle.radius;
        }

        public bool IsInside(Vector2 point)
        {
            return Calculate.Distance(this.center, point) < this.radius;
        }

        public bool IsInside(float x, float y)
        {
            return Calculate.Distance(this.center, new Vector2(x, y)) < this.radius;
        }

        public static Circle RotateAround(Circle circle, Vector2 origin, float radians)
        {
            return new Circle(Calculate.RotateAround(circle.center, origin, radians), circle.radius);
        }

        public void RotateAround(Vector2 origin, float radians)
        {
            this.center = Calculate.RotateAround(this.center, origin, radians);
        }
        ...
        {
            float num = System.Math.Max(Calculate.Distance(new Vector2((float)rec.X, (float)rec.Y), new Vector2((float)rec.Right, (float)rec.Bottom)), Calculate.Distance(new Vector2((float)rec.Right, (float)rec.Y), new Vector2((float)rec.X, (float)rec.Bottom)));
        ...
        {
            float num = System.Math.Max(Calculate.Distance(new Vector2(rec.X, rec.Y), new Vector2(rec.Right, rec.Bottom)), Calculate.Distance(new Vector2(rec.Right, rec.Y), new Vector2(rec.X, rec.Bottom)));
